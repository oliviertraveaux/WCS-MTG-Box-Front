...
import {GetQualityClassPipe} from "../../pipes/get-quality-class.pipe";
...
export class CollectionDisplayListComponent implements OnInit {
  ...
  displayedColumns: string[] = ['name', 'quality', 'set', 'rarity', 'offer', 'expand'];
  ...
  ngOnInit() {
    ...
    this.cards$.subscribe((cards: UserCard[]) => {
  ...
  onDelete(userCardId: number): void {
    this._collectionCards.deleteCard(userCardId).subscribe(() => {
      ...
      this.cardsData.data = this.cardsData.data.filter(card => card.userInfo.userCardId !== userCardId);
    });
  }
