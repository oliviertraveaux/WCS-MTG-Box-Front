...
export class ProfilePageComponent implements OnInit {
  ...
  openModifyPasswordDialog() {
    const dialogRef = this._dialog.open(ModalModifypassword, {
      width: '600px'
    });
  }

  openDeleteUserDialog() {
    const dialogRef = this._dialog.open(ModalDeleteUser, {
      width: '600px'
    });
  }
  ...
  changeUsername() {
    ...
    this._alertService.openConfirmDialog(alertTitle, alertInfo).subscribe(result => {
      if (result && this.userNameForm.valid) {
        ...
        if (username) {
          ...
          this._userInfosService.updateUsername(this.userId, username).subscribe({
            next: () => {
              ...
              setTimeout(() => {
                this._authService.loginLogout().subscribe({
                  next: () => {
                    this._router.navigate(['/login']);
                  },
                  error: (err: any) => {
                    console.error('Error during logout:', err);
                    this._alertService.openSnackBar('Logout failed', SnackbarStatus.error);
                  }
                });
              }, 2000);
            },
